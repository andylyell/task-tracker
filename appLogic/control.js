...
function setUpEventListeners() {
  ...
  data.retrieveActiveTasks()
    .then(() => {
      ...
      //================// Search function ================//
      const activeSearch = document.querySelector(ui.DOMstrings.activeTaskBody);
      console.log(activeSearch);

      if(activeSearch.classList.contains('disabled')){
        console.log('yes');
      } else {
        document.querySelector(ui.DOMstrings.searchField).addEventListener('input', () => {
          ui.searchActiveFunction();
        })
      }
  ...
  .then(() => {
    ...
    document.querySelector(ui.DOMstrings.archiveCount).innerHTML = archiveDoc.length;
    archiveDoc.forEach((task) => {
      archiveContainer.innerHTML += `<div class="task-item-container">
